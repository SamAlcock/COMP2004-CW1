    ...
    for (x = 0; x < N; x++)
        for (y = 0; y < M; y++)
            if (y >= C0 && y <= M - 1 - C0 && x >= C0 && x <= N - 1 - C0) {
                ...
                x_compute[x][y][C0] = x_compute[x][y][C0 - 1] + input[x - 1][y] * Filter[abs(-1)];
                x_compute[x][y][1 + C0] = x_compute[x][y][C0] + input[x][y] * Filter[abs(0)];
                x_compute[x][y][2 + C0] = x_compute[x][y][C0 + 1] + input[x + 1][y] * Filter[abs(1)];
    ...
    for (x = 0; x < N; x++)
        for (y = 0; y < M; y++)
            if (y >= C0 && y <= M - 1 - C0 && x >= C0 && x <= N - 1 - C0) {
                ...
                xy_compute[x][y][C0] = xy_compute[x][y][C0 - 1] + x_image[x][y - 1] * Filter[abs(-1)];
                xy_compute[x][y][C0 + 1] = xy_compute[x][y][C0] + x_image[x][y] * Filter[abs(0)];
                xy_compute[x][y][C0 + 2] = xy_compute[x][y][C0 + 1] + x_image[x][y + 1] * Filter[abs(1)];
